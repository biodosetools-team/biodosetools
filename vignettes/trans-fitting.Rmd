...
Once the genomic conversion factor has been calculated, we can input the count data. On the \pkg{shiny} app, we can select to either load the count data from a file (supported formats are `.csv`, `.dat`, and `.txt`) or to input the data manually. Once the table is generated and filled, the "Calculate parameters" button will calculate the number of cells ($N$), number of aberrations ($X$), as well as mean ($\bar{y}$), variance ($\hat{\sigma}^{2}$), dispersion index ($\hat{\sigma}^{2}/\bar{y}$), and $u$-value.
...
```{r trans-fit-dose-curve, fig.width=6, fig.height=3.5, fig.align='center'}
plot_fit_dose_curve(
  fit_results,
