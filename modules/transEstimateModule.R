        ...
        fluidRow(
          column(
            ...
            conditionalPanel(
              ...
              numericInput(ns("num_trans"), "Maximum number of translocations per cell", value = 5)
            ),
            conditionalPanel(
              condition = "input.load_case_data_check",
              ns = ns,
...
transEstimateHotTable <- function(input, output, session, stringsAsFactors) {
  ...
  previous <- reactive({
    ...
    isolate({
      ...
      num_cases <- 1
      num_trans <- as.numeric(input$num_trans) + 1
    })
    ...
    if (!load_case_data) {
      ...
      full_data <- data.frame(
        ...
        `colnames<-`(paste0("C", seq(0, num_trans - 1, 1)))
...
transEstimateFittingCurve <- function(input, output, session, stringsAsFactors) {
...
transEstimateResults <- function(input, output, session, stringsAsFactors) {
  ...
  data <- reactive({
    ...
    gg_curve <- ggplot(curves_data) +
      ...
      labs(x = "Dose (Gy)", y = "Translocations/cells") +
      theme_bw()
  ...
  output$save_report <- downloadHandler(
    ...
    filename = function() {
      paste("translocations-estimate-report-", Sys.Date(), ".html", sep = "")
    },
    content = function(file) {
      ...
      file.copy("reports/translocations-estimate-report.Rmd", tempReport, overwrite = TRUE)
