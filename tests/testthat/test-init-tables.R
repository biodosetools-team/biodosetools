...
test_that("init_aberr_table for dicentrics case data works", {
  ...
  # Expected outputs
  expect_equal(names(full_data), c("ID", "N", "X", "C0", "C1", "C2", "C3", "C4", "C5", "y", "y_err", "DI", "u"))
  expect_equal(unname(unlist(full_data[, 2:ncol(full_data)])), rep(0, num_cases * 12))
  expect_true(all(dim(full_data) == c(num_cases, 13)))
})
...
test_that("init_aberr_table for translocations case data works", {
  ...
  # Expected outputs
  expect_equal(names(full_data), c("ID", "N", "X", "C0", "C1", "C2", "C3", "C4", "C5", "Fp", "Fp_err", "DI", "u", "Xc", "Fg", "Fg_err"))
  expect_equal(unname(unlist(full_data[, 2:ncol(full_data)])), rep(0, num_cases * 15))
  expect_true(all(dim(full_data) == c(num_cases, 16)))
})
