...
```{r irr-cond, echo=FALSE}
irr_conditions <- params$curve_data

irr_table <- purrr::map_dfr(irr_conditions, ~data.frame(
  Label = as.character(.x[["label"]]),
  Value = as.character(.x[["text"]]),
...
`r if (assessment == "partial-body") {"### Yield (dics/cells)"}`
